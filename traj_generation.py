...
class Traj_Gen:
	...
	def __init__(self):
		...
		self.radius = 0.064 #2  #radius of sanding pad inches (in meters)
		...
		self.equal_spacing = 0.0508 #0.00757
	...
	def plot_surface(self):
		...
		self.ax.plot_trisurf(self.data[:,0],self.data[:,1],self.data[:,2], alpha=0.5, color='b',edgecolor='none')
		self.ax1.plot_trisurf(self.data[:,0],self.data[:,1],self.data[:,2], alpha=0.5, color='b',edgecolor='none')
	...
	def base_pts(self):
		...
		while (self.cover_area_tot - self.overlap_area_tot) < 0.30:
	...
	def base_pts_equal(self):
		...
		for k in range(self.num_rows):
			...
			for i in range(prev_max_y_array.shape[0]-1):
				...
				if pt4[1] > pt1[1]:
					uncovered_area = self.area_slice(overlap_pts, self.plot_area, 'g')
	...
	def area_slice(self, pts, plot, color):
		...
		if plot == True:
			if color == 'b':
				self.ax.add_collection3d(Poly3DCollection(vertices, facecolors=[color], alpha=0.5))
			else:
